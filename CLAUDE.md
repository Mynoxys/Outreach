...
- **Backend is Supabase** (Postgres), not a local server. `src/api.ts` creates the client from `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` and maps the tables (`parents`, `journal`, `followups`, `message_counts`, `checklist`, `leads`, `templates`) to/from the `DB` shape. `db/schema.sql` is the source of truth for tables + RLS; newer tables (`checklist`, `leads`, `templates`) are read **tolerantly** in `getState` (a missing table → `[]`) so the app keeps working before a migration is run. Watch the mapping seams: followups `parent_id` ⇄ `parentId`, and `message_counts` rows ⇄ the `messageCounts` object.
...
- **Leads & Templates tabs.** *Leads* = a `leads` backlog of people not yet contacted; "Move to pipeline" (`api.promoteLead`) creates a `cold_sent` parent and deletes the lead. *Templates* = editable copy-paste scripts in a `templates` table (copy / inline-edit / delete); the starter set is `src/lib/starterTemplates.ts`, seeded via "Load starter messages" (and Load sample).
- **`server/` is legacy** — the previous Express + `server/data.json` backend, no longer used (kept for reference only); the live sample-data builder is `src/lib/sampleData.ts`, not `server/sampleData.js`.
...
- *Daily plan* (Plan tab — the default tab) is a fixed sprint routine in `src/lib/playbook.ts` (weekday morning/midday/evening + Mon/Wed/Fri anchors, Sat rest, Sun light). `resolveDay()` in stats.ts marks each task done: "auto" tasks (10 messages, 3 conversations) derive from existing data; manual ticks persist in the `checklist` table. `sprintStats()` computes the funnel against the locked targets in `SPRINT` (types.ts: 240 conversations → 60 started → 30 completed by Aug 22).
