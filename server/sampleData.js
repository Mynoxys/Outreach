// Builds a realistic sample DB so every feature is immediately visible.
// Dates are relative to "now" at call time, so the streak and follow-up
// queue are always fresh whenever sample data is loaded.
...
export function buildSampleData() {
  ...
  const parents = [
    ...
    },
    {
      ...
      status: 'interviewed',
      trial_start_date: null,
      notes: 'Just finished the call (today). Strong fit. Bedtime is her #1 pain. Proposing trial start Monday.',
  ...
  const followups = [
    { id: randomUUID(), parentId: priya.id, type: 'bump1', date: dayKey(-9) },
    { id: randomUUID(), parentId: sam.id, type: 'trial_day3', date: dayKey(-4) },
  ]
